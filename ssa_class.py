...
class SSA(object):
    ...
    def __init__(self,tseries,L,save_mem=True):
        ...
        L: The window length. Must be an interger 2 <= L <= N/2, where N is the length of the time series.
        ...
        Based on https://www.kaggle.com/jdarcy/introducing-ssa-for-time-series-decomposition
    ...
    def reconstruct(self, indices):
        ...
        Parameters
        ----------
        indices: An interger, list of integers or slice(n,m) object, representing the elementary components to sum.
        """
    ...
    def calc_wcorr(self):
        ...
        """
        Calculate the w-correlation matrix for the time series.
        """
    ...
    def plot_eigenvalues(self):
        ...
        ax[0].plot(self.Sigma**2/sigma_sumsq*100,lw=2.5)
        ax[0].set_xlim(0,11)
        ax[0].set_title("Relative contribution of $\mathbf{X}_i$ to trajectory matrix")
        ax[0].set_xlabel("$i$")
        ax[0].set_ylabel("Contribution (%)")
        ax[1].plot((self.Sigma**2).cumsum() / sigma_sumsq*100,lw=2.5)
        ax[1].set_xlim(0,11)
